...
public class NotifyPictureUploadedHandlerTests
{
    [Fact]
    public async Task Handle_Ok()
    {
        ...
        var logger = new Mock<ILogger<NotifyPictureUploadedHandler>>();

        var handler = new NotifyPictureUploadedHandler(hub.Object, logger.Object);

        // Act
        var result = await handler.Handle(new NotifyPictureUploaded(picture), CancellationToken.None);
